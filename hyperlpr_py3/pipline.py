...
def SimpleRecognizePlate(image):
	...
	for j, plate in enumerate(images):
		...
		if len(val) == 3:
			...
			if confidence / 7 > 0.7:
				...
				res_set.append((res, confidence / 7,
								[int(max(0, round(rect[0]))), int(max(0, round(rect[1]))), int(max(0, round(rect[2]))),
								 int(max(0, round(rect[3])))]))
				for i, block in enumerate(blocks):

					block_ = cv2.resize(block, (25, 25))
					block_ = cv2.cvtColor(block_, cv2.COLOR_GRAY2BGR)
					if j * 25 + 25 >= np.size(image, 0) or (i * 25) + 25 >= np.size(image, 1):
						break
	...
	print("Recognize time:", time.time() - t0, "s")
